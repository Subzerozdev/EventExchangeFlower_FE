...
function SellerForm() {
    ...
    const onFinish = async (values: SellerFormValues) => {
        try {
            ...
            setUser((prevUser) => ({
                ...prevUser,
                role: "seller",  // Cập nhật role thành seller
            }));
    ...
    return (
        <Form form={form} layout="vertical" onFinish={onFinish}>
            <Form.Item label="Tên Shop" name="shopName" rules={[{ required: true, message: "Vui lòng nhập tên shop!" }]}>
                <Input />
            </Form.Item>

            <Form.Item label="Địa chỉ Shop" name="shopAddress" rules={[{ required: true, message: "Vui lòng nhập địa chỉ shop!" }]}>
                <Input />
            </Form.Item>
